<?php
class ezjscInformationCollectorCallFunctions
{
    public static function getForm( $functionName ){
    	$Module = eZModule::findModule("content");
	    $http = eZHTTPTool::instance();
		$tpl = eZTemplate::factory();
		$ini = eZINI::instance();
		
		$viewParameters = array("ajax" => true);
		...
		$id_type = $http->hasPostVariable('id_type') ? $http->postVariable('id_type') : "node_id";
		$id = $http->hasPostVariable('id') ? $http->postVariable('id') : 0;
		
		switch ($id_type) {
			case "remote_object_id":
				$object = eZContentObject::fetchByRemoteID($id);
				if(!$object)
					return $Module->handleError( eZError::KERNEL_NOT_AVAILABLE, 'kernel' );
				
				$NodeID = $object->mainNodeID();
			break;
    ...
    public static function checkForm( $functionName ){
    	$Module = eZModule::findModule("content");
    	$http = eZHTTPTool::instance();
    	$userParameters = array("ajax" => true);
    	...
    	if ( $canCollect )
    	{
    		...
    		else
    		{
    			...
    			if ( $display == 'node' )
    			{
    				...
    				return $Module->run( 'view', array( $ViewMode, $NodeID ),
    											array( 'ViewCache' => false,
    				    				    			'AttributeValidation' => array( 'processed' => true,'attributes' => $unvalidatedAttributes ),
    			...
    			{
    				return $Module->run( 'collectedinfo', array( $NodeID ),
    														array( 'ViewCache' => false,
